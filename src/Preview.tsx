...
class Preview extends React.PureComponent<IPreviewProps> {
  render() {
    ...
    if (status === 'error_file_size' || status === 'error_validation') {
      return (
        <div
          className={`${className} error sm:flex sm:items-center sm:justify-between lg:flex-row flex-col relative block w-full border-2 border-red-300 p-12 text-center bg-red-50 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500`}
          style={style}
        >
          <div className="dzu-previewFileNameError">{title}</div>
          {status === 'error_file_size' && (
            <span className="text-lg text-red-900 font-semibold">
          ...
          {canRemove && (
            <button
              type="button"
              className="dzu-previewButton remove inline-flex items-center shadow-sm px-2.5 py-0.5 border border-gray-300 text-sm leading-5 font-medium rounded-full text-gray-700 bg-white hover:bg-gray-50 self-center"
              onClick={remove}
            >
              Remove
            </button>
    ...
    return (
      <div
        className={`${typeClassName} lg:flex flex-col lg:items-center lg:justify-between md:block  p-3 bg-gray-100 shadow rounded max-w-2xl mx-auto px-4`}
        style={style}
      >
        {previewUrl && (
        ...
        <div className="dzu-previewStatusContainer  flex items-center flex-inline justify-between block w-full">
          {isUpload && (
            <div className="progressDetailsContainer  flex items-center flex-inline justify-between">
              ...
              <span
                className={`${status == 'aborted' ? 'text-red-500' : 'text-indigo-500'} status`}
          ...
          <div
            className="btn-toolbar flex flex-col items-center space-y-3"
            role="toolbar"
          ...
          >
            {status === 'uploading' && canCancel && (
              <button
                type="button"
                className="cancel ml-3 inline-flex justify-center py-1 px-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500"
                onClick={cancel}
              >
                Cancel
              </button>
            ...
            {status !== 'preparing' && status !== 'getting_upload_params' && status !== 'uploading' && canRemove && (
              <button
                type="button"
                className="remove ml-3 inline-flex justify-center py-1 px-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                onClick={remove}
              >
                Remove
              </button>
            ...
            {['error_upload_params', 'exception_upload', 'error_upload', 'aborted', 'ready'].includes(status) &&
              canRestart && (
                <button
                  type="button"
                  className="restart ml-3 inline-flex justify-center py-1 px-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                  onClick={restart}
                >
                  Upload
                </button>
